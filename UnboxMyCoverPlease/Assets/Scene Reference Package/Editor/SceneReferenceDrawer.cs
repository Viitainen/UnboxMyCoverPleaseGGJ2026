...
namespace FrostBit
{
    ...
    public class SceneReferencePropertyDrawer : PropertyDrawer
    {
        ...
        /// <summary>
        /// Class to help with drawing of the GUI-elements
        /// </summary>
            ...
            public static void AddBuildScene(BuildScene buildScene, bool force = false, bool enabled = true)
            {
                if (force == false)
                {
                    int selection = EditorUtility.DisplayDialogComplex(
                        ...
                        "Add as Enabled",       // option 0 (confirm)
                        "Cancel (do nothing)",  // option 1 (cancel)
                        "Add as Disabled");     // option 2 (alt)
            ...
            public static void RemoveBuildScene(BuildScene buildScene, bool force = false)
            {
                ...
                if (force == false)
                {
                    ...
                    if (buildScene.scene.enabled)
                    {
                        ...
                        selection = EditorUtility.DisplayDialogComplex(title, details, confirm, cancel, alt);
                        Debug.Log(selection);
                    }
                    else
                    {
                        selection = EditorUtility.DisplayDialog(title, details, confirm, cancel) ? 0 : 1;
                    }
